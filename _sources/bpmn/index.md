...
A BPMN sequence flow consists of at least one {bpmn}`start-event` **start event**, one {bpmn}`end-event` **end event**, and any number of {bpmn}`task` **tasks** (BPMN activities) connected between them. {download}`sequence-flow.bpmn`
```
...
In this example, a business error is expected with a {bpmn}`bpmn-error-boundary-event` **error boundary event** (which is always interrupting), and it is used to route the process to an alternative {bpmn}`error-end-event` **business error end event** for further rerouting in the calling parent process. {download}`boundary-bpmn-error.bpmn`
```

## Embedded sub-process

```{bpmn-figure} embedded-subprocess
An **embedded sub-process** is a process with its own {bpmn}`start-event` **start event** and {bpmn}`end-event` **end event(s)** within its host process. It is a powerful pattern for wrapping tasks that should share boundary events. In this example, an {bpmn}`interrupting-timer-boundary-event` **interrupting boundary timer event** is used to cancel the whole sub-process. {download}`embedded-subprocess.bpmn`
```
...
A {bpmn}`call-activity-task` **call activity** is used to encapsulate and reference a reusable sub-process (or another process), allowing for modular and maintainable process designs. In this example, it is used to hide the embedded sub-process details from the earlier examples.
...
A **service task** {bpmn}`service-task` represents an automated task. All {bpmn}`robot-task` **Robot Framework** tests and tasks are modeled as service tasks.
```
...
Now that {bpmn}`service-task` **service task** has become the core component in process automation, it has also become common to customize its symbol to make it easier to recognize service tasks by category. So, when you see a task element with a unique symbol, like the {bpmn}`robot-task` **Robot Framework** logo, it is safe to assume that it is a service task with a custom symbol.
```
